...
async function getLogs(podName, namespace) {
  ...
  try {
    ...
    const logs = await k8sApi.readNamespacedPodLog(podName, namespace, undefined, false, undefined, undefined, false, false, 300, 30, true);
...
app.post(
  '/deletePod',
  (req, res) => {
    k8sApi.deleteNamespacedPod(`${req.params.name}`, `${req.params.namespace}`);
...
app.get(
  ...
  (req, res) => {
    res.header('Access-Control-Allow-Origin', '*');
    k8sApi.listNode(undefined, undefined, undefined, undefined, `operatorid=${req.params.pubkey}`)
    .then((response) => {
      nodeobject = [];
      id = 1;
      for (const item of response.body.items) {
        ...
        node["time"] = ((new Date() - item.metadata.creationTimestamp)/3600000).toFixed(2) + " hours";
        id++
